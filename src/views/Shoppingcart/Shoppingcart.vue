  ...
  <div>
    <div>
      ...
      <div v-if="shopList.length <= 0">
        ...
        <div class="cart-item5">你的购物车还是空的</div>
        <div class="cart-item5" @click="buyShop">
          <div class="cart-item4">购物</div>
        </div>
      </div>
      <div v-else>
        <div class="chose">
          <div v-if="checkAll === false">
            <van-checkbox v-model="checkAll" @click="checkedAll"
              >全选</van-checkbox
            >
          </div>
        ...
        <div class="van-btn">
          <van-button class="btn" @click="del" color="#FFA07A">删除</van-button>
          <van-button @click="Goto" color="#4B0082">结算</van-button>
        </div>
        <div class="item">
...
export default {
  ...
  methods: {
    ...
    Goto() {
      ...
      this.$router.push({
        name: "Order",
        query: { msg: this.msg, total: this.total, flagy: this.flagy },
      });
    },
  },
  mounted() {
...
.cart-item4 {
  border: 1px solid rgb(184, 164, 164);
  border-radius: 5px;
  padding: 8px 14px;
}
</style>
