<template>
 ...
 <div class="box1">
  ...
  <van-swipe-item  :width="125" v-for="(item,index) in recommendation" :key="index">
  <div class="box-1"   @click="init(index)">
  ...
  <div class="dis-1">
      <div class="box-3"><i class="lbw iconfont icon-gouwuche"></i></div>
      <div class="box-4 dis-1 al-1 ju-1">查看详情</div>
  </div>
...
<script>
 export default {
   ...
   methods: {
    init(index){
        this.$router.push({name:'Details',query:{id:this.recommend[index].goods.Id},})
        console.log(this.recommend[index].goods.Id);
    }
   },
   mounted() {
