    ...
    <div class="dis-1">
      ...
      >
        <van-tab v-for="(item,index1) in bxMallSubDto" :key="index1" :title="item.mallSubName">
          <div v-for="(item,index) in dataList" :key="index" @click="goDetail(item,index)">
            <van-card
              :title="item.name"
...
export default {
  ...
  methods: {
    // 跳转详情页
    goDetail(item,index) {
      this.$router.push({ name: "Details", query: { id: this.bxMallSubDto[item].goodsId } });
      this.$utils.goDetail(this.bxMallSubDto[index])
    },
