<template>
  <div>
    ...
    <div class="box2 w-1 dis-1">
      ...
      <div class="jack">
        <van-icon name="thumb-circle-o" @click="logo" />
        <div class="fz-14 mt-10" @click="logo">评价</div>
      </div>
      <div class="jack">
    ...
    <div class="box3 po-1" @click="cnpm">
      <div class="icon"><van-icon name="records" badge="lbw.length"/> 全部订单</div>
      <div class="icon1 po-2">〉</div>
    </div>

    <div class="box4">
      <div class="box-1 dis-1 al-1  po-1" @click="mata">
        <div class="fz-14"><van-icon name="star-o" /> 收藏商品</div>
        <div class="icon1 po-2">〉</div>
      </div>

      <div class="box-1  dis-1 al-1  po-1" @click="Add">
        <div class="fz-14"><van-icon name="location-o" /> 地址管理</div>
        <div class="icon1 po-2">〉</div>
      </div>

      <div class="box-2 dis-1 al-1  po-1" @click="Recent">
        <div class="fz-14"><van-icon name="eye-o" /> 最近浏览</div>
        <div class="icon1 po-2">〉</div>
      </div>
    </div>
  </div>
...
export default {
  ...
  mounted() {
    let lbw = ''
    this.lbw = this.$route.query.list
    this.$api
      .queryUser()
      .then((res) => {
